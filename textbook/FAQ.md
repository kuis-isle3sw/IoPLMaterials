...
`opam init` を実行すると，途中で `.bashrc` 等の設定ファイルを書き換えて毎回 `eval $(opam env)` を勝手に実行してよいか？と尋ねるプロンプトが出てきます．これを無視してエンターキーを押すと毎回 `eval $(opam env)` を実行しないといけなくなります．もう一度 `opam init` を実行して，出てくるプロンプトに `y` を入力すると良いかもしれません．
